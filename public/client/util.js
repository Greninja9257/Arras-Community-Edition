...
const util = (function () {
  return {
    ...
    Smoothbar: (value, speed, sharpness = 3, lerpValue = 0.05, syncWithfps = false) => {
      let time = Date.now()
      let display = value
      let oldvalue = value
      return {
    ...
    animBar: class {
      ...
      get (a, b = !1) {
        if (config.animationSettings.value === 0) return this.value
        if (this.h === 0) return 0
        if (this.h === 1) return (this.h = 2), (this.time = a), this.D
        a -= this.time
        let d = 1e3 / config.roomSpeed / 30
        config.animationSettings.value === 2
          ? ((b = a / d), (b = b > 8 ? 8 : b > 1 ? b : b * b * (3 - 2 * b)))
          : (b = a < d || b ? a / d : 1)
        a = this.D
        d = this.value
        if (this.O) {
    ...
    getEntityImageFromMockup: (index, color) => {
      const firstIndex = parseInt(index.split('-')[0])
      let mainMockup = global.mockups[firstIndex]
      ...
      const allRoots = []
      let trueColor = mainMockup.color
      ...
      return {
        ...
        turrets: turrets.map((t) => {
          const o = util.getEntityImageFromMockup(t.index)
          o.color = t.color
    ...
    sizeMultipleMockups: (positionData) => {
      ...
      function checkIfOnLine (endpoint1, endpoint2, checkPoint) {
        ...
        const yDiff = endpoint2[1] - endpoint1[1]
        ...
        const xLengthToCheck = checkPoint[0] - endpoint1[0]
        const predictedY = endpoint1[1] + xLengthToCheck * slope
      ...
      function constructCircumcirle (point1, point2, point3) {
        ...
        const x1 = rounder(point1[0])
        const y1 = rounder(point1[1])
        const x2 = rounder(point2[0])
        const y2 = rounder(point2[1])
        let x3 = rounder(point3[0])
        const y3 = rounder(point3[1])
      ...
      for (const position of positionData) {
        const { axis, middle } = position
        for (let i = 0; i < 32; i++) {
          const theta = Math.PI / 16 * i
      ...
      const point1 = getFurthestFrom(0, 0)
      let point2 = getFurthestFrom(...point1)
      ...
      const avgX = (point1[0] + point2[0]) / 2
      const avgY = (point1[1] + point2[1]) / 2
      let point3 = getFurthestFrom(avgX, avgY)
    ...
    requestEntityImage: (data, color = '16 0 1 0 false') => {
      ...
      let image = {}
      for (const index of typeof data === 'string' || data instanceof String ? data.split('-') : data.index.split('-')) {
