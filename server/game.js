...
class gameServer {
  ...
  defineRoom () {
    ...
    if (!this.wallGrid) {
      this.room.wallGrid = {
        ...
        getGrid: (location) => {
          ...
          return {
            x: (x + 0.5) / this.room.wallGrid.xgrid * this.room.wallGrid.width - this.room.wallGrid.width / 2,
            y: (y + 0.5) / this.room.wallGrid.ygrid * this.room.wallGrid.height - this.room.wallGrid.height / 2,
    ...
    this.room.isAt = (location) => {
      ...
      return {
        x: (x + 0.5) / this.room.xgrid * this.room.width - this.room.width / 2,
        y: (y + 0.5) / this.room.ygrid * this.room.height - this.room.height / 2,
  ...
  close (spawnTimeout) {
    ...
    this.arenaClosed = true
    for (const entity of entities.values()) if (entity.isPlayer || entity.isBot) entity.kill() // Kill all players and bots.
    setTimeout(() => {
      // Wipe everyone
