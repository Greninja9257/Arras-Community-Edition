    ...
    if (info.PROPERTIES != null) {
      if (info.PROPERTIES.COLOR != null) {
        if (typeof info.PROPERTIES.COLOR === 'number' || typeof info.PROPERTIES.COLOR === 'string') {
          if (!isNaN(info.PROPERTIES.COLOR) && !isNaN(parseFloat(info.PROPERTIES.COLOR)) || /^[a-zA-Z]*$/.test(info.PROPERTIES.COLOR)) { this.colorUnboxed.base = info.PROPERTIES.COLOR }
  ...
  define (def) {
    ...
    if (set.COLOR != null) {
      if (typeof set.COLOR === 'number' || typeof set.COLOR === 'string') {
        if (!isNaN(set.COLOR) && !isNaN(parseFloat(set.COLOR)) || /^[a-zA-Z]*$/.test(set.COLOR)) { this.colorUnboxed.base = set.COLOR }
...
class MockupEntity {
  ...
  set (vclass) {
    ...
    if (set.COLOR != null) {
      if (typeof set.COLOR === 'number' || typeof set.COLOR === 'string') { this.colorUnboxed.base = set.COLOR } else if (typeof set.COLOR === 'object') {
        this.colorUnboxed = {
          base: set.COLOR.BASE ?? 16,
          hueShift: set.COLOR.HUE_SHIFT ?? 0,
          saturationShift: set.COLOR.SATURATION_SHIFT ?? 1,
    ...
    for (const def of vclass) this.defs.push(def)
    ...
    if (set.PROPS != null) {
      ...
      for (let i = 0; i < set.PROPS.length; i++) {
        ...
        const o = new MockupEntityProp(def, this)
