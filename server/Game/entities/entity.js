...
class Entity extends EventEmitter {
  constructor (position, master) {
    ...
    for (const v of global.gameManager.views) v.add(this)
  ...
  define (defs, emitEvent = true, overwriteindex = true, overrideDefs = true) {
    ...
    this.props.clear()
    this.store = {}
    for (const gun of this.guns.values()) gun.store = {}
    if (set.PARENT != null) {
    ...
    if (typeof set.defineLevelSkillPoints === 'function') this.skill.LSPF = set.defineLevelSkillPoints
    ...
    if (set.RESET_EVENTS) {
      for (const { event, handler, once } of this.definitionEvents) this.removeListener(event, handler, once)
      this.definitionEvents = []
    }
    if (set.REROOT_UPGRADE_TREE) this.rerootUpgradeTree = set.REROOT_UPGRADE_TREE
    if (Array.isArray(this.rerootUpgradeTree)) {
      let finalRoot = ''
      for (const root of this.rerootUpgradeTree) finalRoot += root + '\\/'
    ...
    if (set.TURRETS != null) {
      for (const turret of this.turrets.values()) turret.destroy()
      this.turrets.clear()
      for (let i = 0; i < set.TURRETS.length; i++) {
        const def = set.TURRETS[i]
        const o = new turretEntity(def.POSITION, this, this.master)
  ...
  refreshBodyAttributes () {
    ...
    const speedReduce = Math.min(2, this.size / (this.coreSize || this.SIZE))
  ...
  camera () {
    ...
    const layerValue = this.layerID || (this.bond != null
      ? this.bound.layer
      : (this.type === 'wall'
          ? 11
          : this.type === 'food'
  ...
  syncTurrets () {
    for (const gun of this.guns.values()) gun.syncChildren()
  ...
  syncSkillsToGuns () {
    for (const gun of this.guns.values()) gun.syncChildren()
    for (const turret of this.turrets.values()) turret.syncTurrets()
  }
  ...
  upgrade (number, branchId) {
    ...
    if (number.isDailyUpgrade) {
      ...
      if (number < this.upgrades.length && this.skill.level >= this.upgrades[number].level) {
        upgraded = true
        const upgrade = this.upgrades[number]; const upgradeClass = upgrade.class; const upgradeBranch = upgrade.branch; const redefineAll = upgrade.redefineAll
        if (redefineAll) {
  ...
  friction () {
    ...
    const excess = motion - this.maxSpeed
    if (excess > 0 && this.damp) {
      const k = this.damp / global.gameManager.roomSpeed
      const drag = excess / (k + 1)
      const finalvelocity = this.maxSpeed + drag
  ...
  destroy () {
    ...
    // Remove from minimap
    const i = global.gameManager.minimap.findIndex(entry => { return entry[0] === this.id })
