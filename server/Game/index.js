class gameHandler {
    ...
    if (this.checkUsers() && Config.bosses_spawn && !this.naturallySpawnedBosses.length && this.bossTimer++ > Config.boss_spawn_cooldown) {
      ...
      setSyncedTimeout(() => {
        ...
        for (let i = 0; i < amount; i++) {
          let spot; let attempts = 30; const name = names[i]
  ...
  quickMaintainLoop = () => {
    ...
    if (!global.gameManager.arenaClosed && !global.cannotRespawn && this.bots.length < Config.bot_cap) {
      const team = Config.mode === 'tdm' || Config.mode === 'tag' ? getWeakestTeam(global.gameManager) : undefined
  ...
  spawnBots (loc, team) {
    ...
    if (Config.clan_wars) {
      const clanTag = ran.chooseClanName()
    ...
    let color
    if (Config.clan_wars) {
      const playerInfo = Config.clan_wars_ft.getPlayerInfo(botName)
