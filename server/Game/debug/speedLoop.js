...
class speedcheckloop {
  ...
  update () {
    ...
    const collidetime = logs.collide.sum()
    const movetime = logs.entities.sum()
    const maptime = logs.minimap.sum()
    const physicstime = logs.physics.sum()
    const lifetime = logs.life.sum()
    const thoughtime = logs.though.sum()
    const selfietime = logs.selfie.sum()
