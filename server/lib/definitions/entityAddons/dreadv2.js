...
function mergeHexnoughtWeaponV2 (weapon1, weapon2) {
  ...
  const GUNS = []
  const gunsOnOneSide = []
  const weapon2GunsOnOneSide = []
  const TURRETS = []
  const turretsOnOneSide = []
  const weapon2TurretsOnOneSide = []

  // Label
  const name1 = hexDreadNames[weapon1.LABEL][weapon2.LABEL]
  const name2 = hexDreadNames[weapon2.LABEL][weapon1.LABEL]
  let weaponName = weapon1.LABEL + weapon2.LABEL
  let orientationId = 0
  if (name1) {
    weaponName = name1
  } else if (name2) {
  ...
  const className = weapon1.LABEL.toLowerCase() + weapon2.LABEL + orientationId + 'OfficialV2'
  ...
  for (const g in weapon2GunsOnOneSide) weapon2GunsOnOneSide[g].POSITION[5] += 60
  ...
  for (const t in weapon2TurretsOnOneSide) weapon2TurretsOnOneSide[t].POSITION[3] += 60
...
function makeHexnoughtBodyV2 (body) {
  ...
  // Body stat modification
  if (body.BODY) for (const m in body.BODY) BODY[m] = body.BODY[m]
