...
global.getWeakestTeam = () => {
  ...
  for (const o of global.entities) {
  ...
  teamcounts = Object.entries(teamcounts).map(([teamId, amount]) => {
    const weight = teamId in Config.team_weights ? Config.team_weights[teamId] : 1
    return [teamId, amount / weight]
  })
  const lowestTeamCount = Math.min(...teamcounts.map(x => x[1]))
  const entries = teamcounts.filter(a => a[1] == lowestTeamCount)
...
global.bringToLife = (() => {
  return my => {
    ...
    // Guns and turrets
    if (my.guns) {
      for (const gun of my.guns.values()) gun.live()
    }
    if (my.turrets) {
      for (const turret of my.turrets.values()) turret.life()
    }
...
global.runMove = (() => {
  return my => {
    ...
    const g = { x: my.control.goal.x - my.x, y: my.control.goal.y - my.y }
    const gactive = (g.x !== 0 || g.y !== 0)
    let engine = { x: 0, y: 0 }
    const a = my.acceleration / global.gameManager.roomSpeed
    switch (my.motionType) {
      ...
      case 'swarm':
        ...
        if (gactive && l > my.size) {
          const desiredxspeed = my.topSpeed * g.x / l
          const desiredyspeed = my.topSpeed * g.y / l
          const turning = Math.sqrt((my.topSpeed * Math.max(1, my.motionTypeArgs.turnVelocity ?? my.range) + 1) / a)
          engine = {
...
global.runFace = (() => {
  return (my) => {
    const t = my.control.target
    const oldFacing = my.facing
    const defaultBound = () => {
      let givenangle
      if (my.control.main) {
        if (my.master.master.isPlayer) {
          const reverse = my.master.master.reverseTargetWithTank ? 1 : my.master.master.reverseTank
          givenangle = Math.atan2(t.y * reverse, t.x * reverse)
        } else {
          givenangle = Math.atan2(t.y, t.x)
        }
        const diff = util.angleDifference(givenangle, my.firingArc[0])
    ...
    switch (my.facingType) {
      ...
      case 'toTarget':
        ...
        if (my.isPlayer) {
          const reverse = my.reverseTargetWithTank ? 1 : my.reverseTank
  ...
  return (defs, branch, set, my, emitEvent) => {
    ...
    if (set.TURRETS != null) {
      for (let i = 0; i < set.TURRETS.length; i++) {
        ...
        const o = new turretEntity(def.POSITION, my, my.master)
    ...
    if (set.PROPS != null) {
      for (let i = 0; i < set.PROPS.length; i++) {
        ...
        const o = new Prop(def.POSITION, my.master, true)
        const type = Array.isArray(def.TYPE) ? def.TYPE : [def.TYPE]
        for (let j = 0; j < type.length; j++) {
          o.define(type[j])
        }
    ...
    if (Array.isArray(my.rerootUpgradeTree)) {
      let finalRoot = ''
      for (const root of my.rerootUpgradeTree) finalRoot += root + '\\/'
...
global.handleBatchUpgradeSplit = (() => {
  function chooseUpgradeFromBranch (remaining, my) {
    if (remaining > 0) { // If there's more to select
      const branchUgrades = my.tempUpgrades[my.defs.length - remaining]
...
global.makeHitbox = wall => {
  ...
  const distance = Math.sqrt(_size ** 2 + _size ** 2)
...
global.becomeBulletChildren = (socket, player, exit, newgui) => {
  const a = player.body.bulletchildren[player.body.bulletchildren.length - 1]
  if (a !== undefined && a !== null) {
    ...
    let removedchildren = player.body.bulletchildren
...
global.loadAllMockups = (logText = true) => {
  const mockupsLoadStartTime = performance.now()
  if (logText) console.log('Started Loading All Mockups...')
  for (const k in Class) buildMockup(k, false)
  const mockupsLoadEndTime = performance.now()
      ...
      Array(6)
        ...
        .map((_, j, b) => [
          ...
          Array(6)
            .fill()
            .map((_, k, c) => [
              // Chance of spawning, set to large value for regular polygons and exponents otherwise
