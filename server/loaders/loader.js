...
for (const file of requires) {
  ...
  for (const key in module) if (module.hasOwnProperty(key)) global[key] = module[key]
}

// Define room loader
const fs = require('fs')
const path = require('path')
const groups = fs.readdirSync(path.resolve(__dirname, '../Game/room_setup/tiles/'))
const loadRooms = (log = false) => {
